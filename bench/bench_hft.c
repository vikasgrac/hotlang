// bench_hft.c — hotlang vs C++ (default AND tuned) vs Rust on the same HFT
// kernels, same LLVM backend.
//
// Columns:
...
//         hotout/ref_cppt.o hotout/libref_rs.a -o hotout/bench_hft
//   ./hotout/bench_hft
...
static double A[256], B[256], OUT[256];
static double BID[64], ASK[64], PX[64], SZ[64];
static double M[1024], V[32], MV[32];
...
static void row(const char* name, double hot, double cpp, double cppt, double rs) {
    printf("%-14s %8.2f %9.2f %9.2f %9.2f   %5.2fx %5.2fx %5.2fx\n", name, hot,
           cpp, cppt, rs, cpp / hot, cppt / hot, rs / hot);
}
